...
def leg_value(cfg: dict, key: str, leg: str | None):
    """A ``fep`` setting that may be a scalar or a per-leg mapping.

    ``ns_per_window`` and ``equilibration_ns`` are per-leg because the two legs have
    different problems: the unfolded tripeptide reproduces to ~0.1 kcal/mol across
    independent boxes while the folded protein spreads up to 1.46, so sampling them
    equally spends about a third of the compute where there is no defect.

    Raises:
        ValueError: If the value is a per-leg mapping but no leg was supplied, or the
            mapping has no entry for this leg. Silently falling back to a default here
            would produce a plausible, wrong ddG.
    """
    val = cfg["fep"][key]
    ...
    if leg is None:
        raise ValueError(
            f"fep.{key} is per-leg ({sorted(val)}) but no leg was given. The caller must "
            "say which leg it is generating, or the two legs would silently share one "
            "sampling length."
        )
    if leg not in val:
        raise ValueError(f"fep.{key} has no entry for leg {leg!r}; has {sorted(val)}.")
    return val[leg]


def window_schedule(cfg: dict, leg: str | None, smoke: bool = False) -> dict:
    """Step counts for one window: the SINGLE source of this arithmetic.

    ``production_mdp`` writes ``nsteps``/``nstdhdl`` from it and ``run_pmx_window``
    recomputes ``discard`` from it. Those two lived as duplicated five-line blocks until
    2026-09-03; edit one and not the other and the discard silently misaligns with what
    was actually written to the .mdp -- a plausible, wrong ddG with no traceback.

    Returns keys ``production_steps``, ``equil_steps``, ``nsteps``, ``nstdhdl``,
    ``discard``. ``discard`` is the number of leading dH/dl records covering
    equilibration; GROMACS emits one at t=0, so kept = nsteps/nstdhdl + 1 - discard.
    """
    fcfg = cfg["fep"]
    dt_ps = float(fcfg["timestep_fs"]) / 1000.0
    if smoke:
        production_steps, frames, equil_steps = 500, 10, 100
    else:
        production_steps = int(round(float(leg_value(cfg, "ns_per_window", leg)) * 1000.0 / dt_ps))
        frames = int(fcfg["frames_per_window"])
        equil_steps = int(round(float(leg_value(cfg, "equilibration_ns", leg)) * 1000.0 / dt_ps))
...
def production_mdp(cfg: dict, window: int, seed: int, smoke: bool = False,
                   leg: str | None = None, sched: dict | None = None) -> str:
    ...
    fcfg = cfg["fep"]
    dt_ps = float(fcfg["timestep_fs"]) / 1000.0
    # `sched` lets run_pmx_window compute the schedule ONCE and use the same object for
    # both the .mdp and the equilibration discard. Two independent calls is how the .mdp
    # and the discard drift apart, which yields a plausible, wrong ddG with no traceback.
    sched = sched if sched is not None else window_schedule(cfg, leg, smoke)
    nsteps, nstdhdl, equil_steps = sched["nsteps"], sched["nstdhdl"], sched["equil_steps"]
...
def run_pmx_window(cfg: dict, variant: str, leg: str, window: int, rep: int,
    ...
    # ONE schedule for this window, shared by the .mdp and the discard below.
    sched = window_schedule(cfg, leg, smoke)
    mdp_text = production_mdp(cfg, window, seed, smoke, leg=leg, sched=sched)
    ...
    discard = sched["discard"]          # same object that wrote prod.mdp above
